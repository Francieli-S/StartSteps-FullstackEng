...
const longestCommonSubSequence2 = (str1, str2) => {
  ...
  // build the 2D array
  const db = Array.from({ length: lengStr1 + 1 }, () => Array(lengStr2 + 1).fill(''))
...
const calculateLCS2 = () => {
  ...
  const result = longestCommonSubSequence2(str1, str2);

  document.getElementById('output-length').innerText = result;
  document.getElementById('output-lcs').innerText = result;
};
